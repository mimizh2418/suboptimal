...
TEST_CASE("Autodiff - Basic Hessian") {
  ...
  const Matrix2v hess = hessian(f, Vector2v{x, y});
  ...
  const auto expected_hess = Eigen::Matrix2d{{2, -2},  //
                                             {-2, 2}};
  CHECK(suboptimal::getValues(hess).isApprox(expected_hess));
}
