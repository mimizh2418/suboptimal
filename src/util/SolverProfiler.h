...
class SolverProfiler {
 public:
  void startIteration() {
    current_iteration_start_time = std::chrono::steady_clock::now();
    if (!started) {
      started = true;
  ...
  void endIteration() {
    ...
    last_iteration_end_time = std::chrono::steady_clock::now();
  ...
  std::chrono::duration<double, std::milli> lastIterationTime() const {
    if (!started) {
      return std::chrono::duration<double, std::milli>{0.0};
    }
 ...
 private:
  ...
  std::chrono::steady_clock::time_point current_iteration_start_time;
  std::chrono::steady_clock::time_point last_iteration_end_time;

  std::chrono::duration<double, std::milli> last_iteration_time{0.0};
